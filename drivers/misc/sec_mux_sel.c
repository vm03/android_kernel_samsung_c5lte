		...
		switch(mux_sel_n) {
			case SEC_MUX_SEL_1:
				if(mux_sel->pdata->mux_sel_1_en) {
					...
					pr_info("%s: [mux_sel_1] level : %d(%d), adc_type : %d\n", __func__, mux_sel_level, gpio_get_value(mux_sel->pdata->mux_sel_1), adc_type);					
				}
				break;
			case SEC_MUX_SEL_2:
				if(mux_sel->pdata->mux_sel_2_en) {
					...
					pr_info("%s: [mux_sel_2] level : %d(%d), adc_type : %d\n", __func__, mux_sel_level, gpio_get_value(mux_sel->pdata->mux_sel_2), adc_type);					
				}
				break;
			case SEC_MUX_SEL_3:
				if(mux_sel->pdata->mux_sel_3_en) {
					...
					pr_info("%s: [mux_sel_3] level : %d(%d), adc_type : %d\n", __func__, mux_sel_level, gpio_get_value(mux_sel->pdata->mux_sel_3), adc_type);					
				}
				break;
			default:
				dev_err(mux_sel->dev,
